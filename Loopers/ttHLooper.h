...
void add_variables(vector<Process*> v, TString tag, vector<TString> syst_labels = {""}) {
  ...
  for (unsigned int j = 0; j < syst_labels.size(); j++) {
      for (int i = 0; i < v.size(); i++) {
        ...
        v[i]->add_histogram("h" + syst_labels[j] + "FCNC_Hut_BDT_NRB", 25, 0, 8);
        v[i]->add_histogram("h" + syst_labels[j] + "FCNC_Hct_BDT_NRB", 25, 0, 8);
        v[i]->add_histogram("h" + syst_labels[j] + "FCNC_Hut_BDT_SMH", 25, 0, 8);
        v[i]->add_histogram("h" + syst_labels[j] + "FCNC_Hct_BDT_SMH", 25, 0, 8);

        v[i]->add_histogram("h" + syst_labels[j] + "FCNC_Hut_BDT_NRB_SRs", 25, 0, 8);
        v[i]->add_histogram("h" + syst_labels[j] + "FCNC_Hct_BDT_NRB_SRs", 25, 0, 8);
